...
from models.utils.common import (add_sos_eos, log_add, add_whisper_tokens,
                                mask_to_bias)
from models.utils.ctc_utils import remove_duplicates_and_blank
from models.utils.mask import (make_pad_mask, mask_finished_preds,
...
def ctc_greedy_searchv2(ctc_probs: torch.Tensor,
                      ctc_lens: torch.Tensor,
                      blank_id: int = 0) -> List[DecodeResult]:
    batch_size = ctc_probs.shape[0]
    maxlen = ctc_probs.size(1)
    ...
    mask = make_pad_mask(ctc_lens, maxlen)  # (B, maxlen)
    topk_index = topk_index.masked_fill_(mask, blank_id)  # (B, maxlen)
    topk_prob = topk_prob.masked_fill_(mask, 0.0)
    
    hyps = [hyp.tolist() for hyp in topk_index]
    token_probs_list = [prob.tolist() for prob in topk_prob]
    
    results = []
    for hyp, token_probs in zip(hyps, token_probs_list):
        # 过滤掉blank和padding位置
        valid_tokens = []
        valid_probs = []
        for token, prob in zip(hyp, token_probs):
            if token != blank_id and prob > 0:  # prob > 0 表示非padding位置
        ...
        final_tokens = []
        final_probs = []
        for i, (token, prob) in enumerate(zip(valid_tokens, valid_probs)):
            if len(final_tokens) == 0 or token != final_tokens[-1]:
                final_tokens.append(token)
                final_probs.append(prob)
        
        r = DecodeResult(final_tokens, token_probs=final_probs)
        results.append(r)
    return results
